...
func TestSyncGreeterConfigsAndAuthDelegatesSharedAuth(t *testing.T) {
	...
	var calls []string
	greeterConfigSyncFn = func(compositor string, logFunc func(string), sudoPassword string) error {
		if compositor != "niri" {
	...
	err := syncGreeterConfigsAndAuth("niri", func(string) {}, sharedpam.SyncAuthOptions{
...
func TestSyncGreeterConfigsAndAuthStopsOnConfigError(t *testing.T) {
	...
	greeterConfigSyncFn = func(string, func(string), string) error {
	...
	err := syncGreeterConfigsAndAuth("niri", func(string) {}, sharedpam.SyncAuthOptions{}, nil)
...
func TestExtractGreeterShellDirFromCommand(t *testing.T) {
	tests := []struct {
		name    string
		command string
		want    string
	}{
		{"shell-dir short flag", "/usr/bin/dms-greeter --command niri -c /home/dev/dank-greeter/quickshell", "/home/dev/dank-greeter/quickshell"},
		{"legacy path flag", "/usr/bin/dms-greeter --command niri -p /usr/share/quickshell/dms-greeter", "/usr/share/quickshell/dms-greeter"},
		{"no override", "/usr/bin/dms-greeter --command niri", ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := extractGreeterShellDirFromCommand(tt.command); got != tt.want {
				t.Fatalf("extractGreeterShellDirFromCommand(%q) = %q, want %q", tt.command, got, tt.want)
			}
		})
	}
}
...
func TestRejectNixOSGreeterMutationBlocksImperativeCommands(t *testing.T) {
	...
	for _, commandName := range []string{"install", "enable", "sync", "uninstall"} {
		t.Run(commandName, func(t *testing.T) {
			root := &cobra.Command{Use: "dms-greeter"}
			mutationCommand := &cobra.Command{Use: commandName}
			root.AddCommand(mutationCommand)
